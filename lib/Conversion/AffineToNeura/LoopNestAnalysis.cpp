    ...
    while (parentOp && !isa<func::FuncOp>(parentOp)) {
      if (auto parentLoop = dyn_cast<affine::AffineForOp>(parentOp)) {
        ...
        if (it != loopMap.end()) {
          ...
          loopInfo->depth =
              loopInfo->parent->depth + 1;  // depth = parent_depth + 1
      ...
      for (Operation &op : body.getOperations()) {
        if (childLoopOps.contains(&op)) {
          if (prevChild && betweenChildren) {
            info->is_perfect_nest =
                false;  // Operations between siblings → imperfect
            break;
          }
          prevChild = &op;
...
llvm::ArrayRef<LoopInfo *> LoopNestAnalysis::getChildLoops(
    affine::AffineForOp loop) const {
  LoopInfo *info = getLoopInfo(loop);
