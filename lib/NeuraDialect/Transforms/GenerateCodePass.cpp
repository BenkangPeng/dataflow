...
static Topology getTopologyFromArchitecture(int per_cgra_rows,
  ...
  Topology topo;
  mlir::neura::Architecture architecture(
      1, 1, mlir::neura::BaseTopology::MESH, per_cgra_rows, per_cgra_columns,
      mlir::neura::BaseTopology::MESH, mlir::neura::TileDefaults{},
      std::vector<mlir::neura::TileOverride>{}, mlir::neura::LinkDefaults{},
...
struct GenerateCodePass
  ...
  void indexIR(func::FuncOp function, SmallVector<Operation *> &data_movs,
    ...
    function.walk([&](Operation *op) {
      ...
      if (isConstant(op)) {
        ...
        // Handles normal operands, including operations with rhs_value
        // attribute.
        ...
        // Handles cases where binary operations have the RHS constant stored as
        // an attribute.
        if (auto rhs_value_attr = op->getAttr(attr::kRhsValue)) {
          std::string rhs_literal =
              extractConstantLiteralFromAttr(rhs_value_attr);
  ...
  // LOCAL if link-less).
  std::pair<StringRef, StringRef> computeDirections(
  ...
  // - If src_reg_step exists: producer writes ONLY to $src_reg (directional
  // port is emitted by egress at first_link_ts).
  ...
  void setProducerDestination(Operation *producer, StringRef producer_direction,
    ...
    if (auto *pi = getInstructionPointer(producer)) {
      if (src_reg_step) {
        // This mov uses src_reg + egress to send on link.time_step. We still
        // must NOT delete existing directional outputs, because the same
        // producer may fan-out to other consumers via other mov paths.
  ...
  struct MovRegSplit {
    std::optional<RegStep>
        src_reg_step;  // last reg with time_step < first_link_time_step
    std::optional<RegStep>
        dst_reg_step;  // first reg with time_step > last_link_time_step
  };
  ...
  MovRegSplit splitMovRegs(const SmallVector<RegStep, 4> &regs,
    ...
    for (const RegStep &r : regs) {
      ...
      if (!out.dst_reg_step && r.time_step > last_link_time_step)
        out.dst_reg_step = r;
    }
    return out;
  }
    ...
    for (size_t i = 1; i < links.size(); ++i) {
      ...
      if (hop_signatures.insert(sig).second) {
        int hop_id = base_mov_id >= 0
                         ...
                         : -1;
        ++hop_counter;
        placeRouterHop(topo, mid_tile, time_step, in, out, /*asCtrlMov=*/IsCtrl,
                       hop_id);
      }
    }
  }
  ...
  SmallVector<std::pair<Operation *, Value>, 2> collectDataMovConsumers(
      Operation *forwarder) {
  ...
  // *data* inputs (sources).
  SmallVector<std::pair<Operation *, Value>, 2> collectCtrlMovConsumers(
      Operation *forwarder, const DenseMap<Value, Operation *> &reserve2phi) {
    SmallVector<std::pair<Operation *, Value>, 2> consumers;
  ...
  // Try register-based rewiring. If cross-tile, emit deposits
  // [incoming_dir]->[$reg] at earliest reg time_step. Returns true if rewiring
  // to $reg was applied to consumers.
  template <bool IsCtrl>
  bool handleRegisterRewiring(Operation *consumer_operation,
    ...
    if (!links.empty()) {
      // Cross-tile: deposit on destination tile at dst_reg_step (post-link
      // register).
      ...
      StringRef incoming_dir =
          topo.invertDir(topo.dirFromLink(links.back().link_id));
      placeDstDeposit(topo, dst_tile, deposit_time_step, incoming_dir,
                      register_id,
      ...
      if (consumer_placement.has_tile) {
        // For CTRL_MOV, the destination register often represents a stateful
        // value (reserve/control) that must be consumed via a local register
        // even if the consumer's time_step is earlier (e.g., prologue reads
        // default, later iterations read updated).
  ...
  void emitMovRoutingInstructions(Operation *forwarder,
    ...
    setProducerDestination(basics.producer, basics.producer_direction,
                           basics.regs, basics.reg_split.src_reg_step);
    if (!basics.links.empty() && basics.reg_split.src_reg_step) {
      ...
      int egress_instruction_id =
          basics.mov_dfg_id >= 0 ? basics.mov_dfg_id * 10000 : -1;
      placeSrcEgress(topo, src_tile, first_link_time_step,
                     basics.producer_direction,
                     basics.reg_split.src_reg_step->regId,
                     /*asCtrlMov=*/IsCtrl, egress_instruction_id);
    }
    size_t hop_counter = 1;
    generateIntermediateHops<IsCtrl>(basics.links, topo, basics.mov_dfg_id,
                                     hop_counter);
  }

  template <bool IsCtrl>
  SmallVector<std::pair<Operation *, Value>, 2> collectMovConsumers(
      Operation *forwarder, const DenseMap<Value, Operation *> &reserve2phi) {
    if constexpr (IsCtrl) {
  ...
  void rewriteMovConsumers(
    ...
    for (const std::pair<Operation *, Value> &consumer_pair : consumers) {
      ...
      if (!handleRegisterRewiring<IsCtrl>(
              ...
              basics.reg_split.dst_reg_step, basics.links, topo,
              basics.mov_dfg_id))
  ...
  void collectHopRewrites(
      func::FuncOp func, const Topology &topology, DfgNodeMap &nodes,
      const DfgEdgeList &original_edges, DfgEdgeList &edges,
    ...
    func.walk([&](Operation *operation) {
      ...
      SmallVector<std::pair<int, int>, 8> hop_tiles;
      SmallVector<int, 8> hop_time_steps;
      // Detect whether this mov has a src_reg_step (reg.time_step <
      // first_link_time_step). If so, an egress instruction exists with
      // id=mov_id*10000 and must participate in edges.
      ...
      if (!link_steps.empty()) {
        ...
        if (has_src_reg_step) {
          ...
          // Add placeholder node now (will be filled/confirmed by
          // injectMaterializedInstructionNodes).
          if (!nodes.count(egress_id)) {
            nodes[egress_id] = DfgNodeInfo{
                isCtrlMov(operation) ? "CTRL_MOV" : "DATA_MOV", coord.first,
                coord.second, link_steps.front().time_step};
          }
        }
      }
      ...
      if (hop_tiles.empty()) {
        ...
        for (size_t i = 0; i < hop_tiles.size(); ++i) {
          ...
          hop_node.time_step =
              (i < hop_time_steps.size()) ? hop_time_steps[i] : -1;
          nodes[node_id] = hop_node;
        }

        if (producer_id >= 0) skip_edges.insert({producer_id, mov_dfg_id});
        ...
        rewrites.push_back(HopRewriteInfo{mov_dfg_id, producer_id, consumer_ids,
                                          hop_tiles, hop_time_steps});
      }

      // Also rewrite edges for single-hop (no intermediate hop nodes) when
      // egress exists: producer -> egress(base) -> mov_id -> consumers so that
      // DFG edges align with instruction ids.
    ...
    for (const HopRewriteInfo &rewrite : rewrites) {
      ...
      // This keeps DFG edges aligned with instruction ids for reg-before-link
      // paths.
      if (nodes.count(egress_id)) {
      ...
      // Always connect the chain to mov_id; if mov_id is later pruned
      // (non-materialized), pruneMovNodesWithoutCoords will bypass it and
      // connect previous directly to consumers.
  ...
  void pruneMovNodesWithoutCoords(
      DfgNodeMap &nodes, DfgEdgeList &edges,
  ...
  ArrayConfig buildArrayConfig(int columns, int rows, int compiled_ii = -1) {
    ...
    for (auto &[tile_key, timestep_map] : tile_time_instructions) {
      ...
      for (auto &[timestep, instruction_vec] : timestep_map)
        for (Instruction &inst : instruction_vec) flat.push_back(inst);
  ...
  static IndexGroups groupByIndexPerIi(
