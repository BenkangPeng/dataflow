...
double OperationCostModel::pattern_cost(
...
std::vector<int> HardwareTemplate::find_mapping(
    const HardwarePattern& pat) const {
  std::vector<int> best_mapping;
  int best_reuse = -1;
  std::vector<int> cur_mapping;
  std::set<int> used_fus;
...
static bool can_reach_via_connections(
    const std::set<std::pair<int, int>>& connections, int from, int to,
    int num_fus) {
