...
struct HardwareMergePass
  ...
  Option<std::string> outputFile{*this, "output",
                                 ...
                                 llvm::cl::init("hardware_config.json")};
  ...
  void runOnOperation() override {
    ...
    collect_supported_operations(patterns, templates, all_dfg_ops,
                                 supported_ops);

    write_hardware_config_json(outputFile.getValue(), patterns, templates,
                               cost_model, execution_plans, supported_ops);
  }
};
