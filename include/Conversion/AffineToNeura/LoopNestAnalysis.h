...
/// Loop information structure - Stores all analysis information for a single
/// loop.
struct LoopInfo {
  ...
  llvm::SmallVector<Operation *, 4>
      operations_before_child;  // Operations before child loops.
  llvm::SmallVector<Operation *, 4>
      operations_after_child;  // Operations after child loops.
...
///
/// Purpose: Provides loop hierarchy information for AffineToNeura pass to
...
class LoopNestAnalysis {
 public:
  ...
  LoopInfo *getLoopInfo(
      affine::AffineForOp loop) const;  // Gets loop information.
  llvm::ArrayRef<LoopInfo *> getTopLevelLoops() const {
    return topLevelLoops;
  }  // Gets top-level loops.
  llvm::ArrayRef<std::unique_ptr<LoopInfo>> getAllLoops() const {
    return allLoops;
  }  // Gets all loops.
  bool isPerfectNest(
      affine::AffineForOp loop) const;  // Checks if perfect nest.
  LoopInfo *getParentLoop(affine::AffineForOp loop) const;  // Gets parent loop.
  llvm::ArrayRef<LoopInfo *> getChildLoops(
      affine::AffineForOp loop) const;  // Gets child loops.
 ...
 private:
  ...
  llvm::SmallVector<std::unique_ptr<LoopInfo>, 8>
      allLoops;  // All loops (owns ownership).
  llvm::SmallVector<LoopInfo *, 4>
      topLevelLoops;  // Top-level loop pointers list.
};
