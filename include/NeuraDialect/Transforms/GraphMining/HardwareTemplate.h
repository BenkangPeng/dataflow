...
//
// - FunctionalUnit (FU): A single hardware unit that executes one operation
// type
// - HardwareTemplate: A collection of FUs with connections supporting multiple
// patterns
...
struct HardwarePattern {
  ...
  std::vector<int> op_levels;  // Topological level for each op (ops at same
                               // level can run in parallel)
  std::vector<std::vector<int>>
      op_preds;  // Predecessors for each op (dependency graph)
  double cost;
...
// Execution stage for a pattern - contains FU indices that can execute in
// parallel.
...
struct HardwareTemplate {
  ...
  std::set<std::pair<int, int>>
      connections;  // FU connections: (from_fu_id, to_fu_id)
  int instances;
  ...
  // Finds an existing FU that can handle the operation, or -1 if none
  // available.
  int find_available_fu(const std::string& op_type,
  ...
  // Checks if two operations are compatible (can potentially share resources in
  // future).
...
// Generates optimized FU connections (removes redundant connections using
