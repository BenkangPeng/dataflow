...
class DfgNode {
 public:
  ...
  const std::vector<DfgEdge*>& getIncomingEdges() const {
    return incoming_edges_;
  }
  const std::vector<DfgEdge*>& getOutgoingEdges() const {
...
class FrequentSubgraph {
 public:
  ...
  // Copy constructor with new frequency (using tag to disambiguate from copy
  // constructor)
  ...
  const std::map<EdgeId, std::pair<NodeId, NodeId>>& getEdges() const {
    return edges_;
  }
...
struct PatternInstance {
  ...
  bool is_on_critical_path =
      false;  // True if all operations are on critical path
};
...
// Result from GraMi mining: pattern + selected instances (split by critical
// path)
struct PatternWithSelectedInstances {
  ...
  std::vector<PatternInstance>
      critical_instances;  // Instances on critical path
  std::vector<PatternInstance>
      non_critical_instances;  // Instances not on critical path
...
class GraMi {
 public:
  ...
  void setCriticalPathOps(
      const llvm::DenseSet<mlir::Operation*>& critical_ops) {
  ...
  static llvm::DenseSet<mlir::Operation*> collectCriticalPathOps(
  ...
  static std::vector<PatternWithSelectedInstances>
  selectPatternsWithCriticalPriority(
 ...
 private:
  ...
  llvm::DenseSet<mlir::Operation*>
      critical_path_ops_;  // Operations on critical paths
  ...
  static bool instancesConflict(const PatternInstance& a,
                                const PatternInstance& b);
  static bool patternsConflict(const PatternWithInstances& a,
  ...
  // Check if an instance is on critical path (all operations must be on
  // critical path)
